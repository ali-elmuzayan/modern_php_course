...
// normalize.css
/*
 * using this file to make browsers render all elements more consistently and
 ...
 *
 * Note1:
 *  - when we are creating a flexbox, by default all the elements will have
 *    exactly the same height
 *    => to solve this use  "align-items"
 *  - when you use display flex so the item will of type flex item even if
 *    you change it to display inline-block or anything flex item will override
 */

// logo
/*
 *  - logo is svg and there is a code in the file so instead of embed it with
 *    the <img> you can directly add the code
 ...
 *  - also instead of using a style as internal you can inline it with element path
 */
...
// image
/*
 * if we want to style an image that user will add so we don't know how the
 * size will be so we need to use theis property
 *  => object-fit: cover;
 * note 1:
 *  - it might not show the whole image but this is the proper way
 *
 * note2 :
 *  - you will need a border radius again instead use overflow:hidden to the parent
 *
 * note 3:
 *  - the image by default take care about the text around it so, you could
 *    see a gap after it (baseline)
 *      => to fix it turn to  display: block;
 *
 * note 4 (img in card) :
 *  - set the height to 2rem for example of fixed value but if the card bigger
 *     set it to the height of the card to solve this use 2 value
 *      => height: 2rem;  and min-height:100%;
 */

// container
/*
 * you can set the container and instead of wrapping it with all the elements
 * wrap it with
...
// Submit button
/*
 *  - we was about to create it as input:submit, but we need to add svg
 ...
 *  - also we used psuedo before and after
 *
 * position: absolute
 ...
 *  - when he naming a class use - between words while when neming a class that
 *    sub element use __ in the additional words
